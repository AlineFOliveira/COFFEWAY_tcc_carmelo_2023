...
if (isset($_POST['submit'])) {
    ...
    if ($senhaNovo !== $confirmarSenha) {
        ...
        $sqlUpdate = "UPDATE adm SET nome_completo = '$nomeNovo', cpf = '$cpfNovo', endereco = '$enderecoNovo', telefone = '$telefoneNovo' WHERE id = $idNovo";

        $resultUpdate = mysqli_query($conexao, $sqlUpdate);

        if ($resultUpdate) {
            // Insere o novo login associado ao ID do cliente
            $sql_login = "UPDATE login SET email = '$emailNovo', senha = '$senhaNovo' WHERE id_adm = $idNovo";

            $result_login = mysqli_query($conexao, $sql_login);
            if ($result_login) {
                // Atualização bem-sucedida
                echo '<script type="text/javascript">alert("Editado com sucesso.");</script>';
                echo '<script type="text/javascript">window.location = "adm.php";</script>';
            } else {
                // Erro ao inserir dados de login
                echo "Erro ao atualizar os dados de login.";
            }
        } else {
            // Erro ao atualizar os dados
            echo "Erro ao atualizar os dados do administrador.";
        }
    }
}
?>
...
<body>
    <form action="" class="login" method="post">
        ...
        <div class="backButton">
            <a href="adm.php"><svg width="51" height="45" viewBox="0 0 51 45" fill="none" xmlns="http://www.w3.org/2000/svg">
